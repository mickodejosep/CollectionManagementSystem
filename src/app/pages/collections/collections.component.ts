...
export class CollectionsComponent {
  ...
  readonly submitLabel = computed(() => (this.editingId() ? 'Update Collection' : 'Add Collection'));
  ...
  submit(): void {
    ...
    if (this.editingId()) {
      this.collectionService.updateCollection(this.editingId()!, payload);
  ...
  delete(record: CollectionRecord): void {
    const confirmed = window.confirm(`Delete collection from ${record.collectorName}?`);
    if (!confirmed) {
      return;
    }

    this.collectionService.deleteCollection(record.id);
